%{
 ...
 with the IMH-GM-based IBIS filter.
%}
...
N_T=0.5*np;         % resampling treshold
...
for i=2:nn
    ...
    if N_eff<N_T || i==nn
        ...
        model_eval=model_eval+(i-1)*np;
        
        np_curr=length(w);
        
        [mu, si, p] = EMGM(part_u',w,nGM);
        part_cand_u=gmm_rand(np, mu, si, p);
        part_cand_x = u2x(part_cand_u);
        
        ind_res=randsample(np_curr,np,true,w);
        logprob_tot=logprob_tot(ind_res);
        part_x=part_x(ind_res,:);
        part_u=part_u(ind_res,:);

        logprob_tot_cand=log(mvnpdf(part_cand_u));
        for j=2:i
            a_cand_cur=state_fun_x(part_cand_x,n(j));
            lh_cand_cur=lh_a(a_cand_cur,a_meas(j-1));
            logprob_tot_cand=logprob_tot_cand+log(lh_cand_cur);
        end
        u_acc=rand(np,1);
        alpha=min(exp(logprob_tot_cand-logprob_tot).*gmm_pdf(part_u, mu, si, p)./gmm_pdf(part_cand_u, mu, si, p),1);
        ind_log=u_acc<alpha;
        part_x(ind_log,:)=part_cand_x(ind_log,:);
        part_u(ind_log,:)=part_cand_u(ind_log,:);
        logprob_tot(ind_log)=logprob_tot_cand(ind_log);
        acc_prob=[acc_prob,sum(ind_log)]; %#ok<AGROW>
