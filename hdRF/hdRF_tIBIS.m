...
log_prob=log(mvnpdf(part_u));

for i=2:nt+1
    
    N_unique(i-1)=length(unique(part_x(:,1)));
    ...
    while q<1
        ...
        log_prob_cur=log_prob+(1-q)*log_lh_cur;
        ...
        if N_eff_cur < N_T
            ...
            log_prob=log_prob+dq*log_lh_cur;
            ...
            ind_res=randsample(N,N,true,w);
            log_prob=log_prob(ind_res);
            log_lh_cur=log_lh_cur(ind_res);
            part_x=part_x(ind_res,:);
            part_u=part_u(ind_res,:);
            
            acc_prob_actual_cur=0;
            acc_prob_alpha_cur=0;
            
            for k=1:burn+1
                model_eval=model_eval+(i-1)*N;
                part_u_cand=gmm_rand(N, mu, si, p);
                part_x_cand = prior.U2X(part_u_cand);
                log_prob_cand=log(mvnpdf(part_u_cand));
                for j=2:i-1
                    log_prob_cand=log_prob_cand+loglikelihood(part_x_cand, ind_sm_sol, std_meas, t(j-1), lnD_s_meas(ind_s,j-1));
                end
                log_lh_cur_cand = loglikelihood(part_x_cand, ind_sm_sol, std_meas, t(i-1), lnD_s_meas(ind_s,i-1));
                log_prob_cand=log_prob_cand+q*log_lh_cur_cand;
            
                u_acc=rand(N,1);
                alpha=min(exp(log_prob_cand-log_prob).*gmm_pdf(part_u, mu, si, p)./gmm_pdf(part_u_cand, mu, si, p),1);
                ind_log=u_acc<alpha;
                
                part_x(ind_log,:)=part_x_cand(ind_log,:);
                part_u(ind_log,:)=part_u_cand(ind_log,:);
                log_prob(ind_log)=log_prob_cand(ind_log);
                log_lh_cur(ind_log)=log_lh_cur_cand(ind_log);
                
                acc_prob_actual_cur=acc_prob_alpha_cur+mean(ind_log);
                acc_prob_alpha_cur=acc_prob_alpha_cur+mean(alpha);
            end
            
            acc_prob_actual=[acc_prob_actual,acc_prob_actual_cur/(burn+1)]; %#ok<AGROW>
            acc_prob_alpha=[acc_prob_alpha,acc_prob_alpha_cur/(burn+1)]; %#ok<AGROW>
